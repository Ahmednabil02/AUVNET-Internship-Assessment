...
class WishlistScreen extends StatelessWidget {
          ...
          if (homeState is HomeLoading) {
            ...
            return BlocBuilder<FavoritesCubit, FavoritesState>(
              builder: (context, favoritesState) {
                final favoriteProductIds = BlocProvider.of<FavoritesCubit>(context).favoriteProductIds;
                final favoriteProducts = favoriteProductIds.map((id) {
                  return homeState.products.data?.productList.firstWhere(
                        (product) => product.id == id,
                    orElse: () => null!,
                  );
                }).where((product) => product != null).cast<Data>().toList();
                ...
                return SingleChildScrollView(
                  child: buildProductGrid(context, favoriteProducts, blockHorizontal, blockVertical, screenWidth),
                );
              },
            );
          } else {
  ...
  Widget buildProductGrid(BuildContext context, List<Data> products, double blockHorizontal, double blockVertical, double screenWidth) {
    return Padding(
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            InkWell(
                              onTap: () {
                                BlocProvider.of<FavoritesCubit>(context).toggleFavoriteStatus(products[index].id);
                              },
                              child: BlocBuilder<FavoritesCubit, FavoritesState>(
                                builder: (context, state) {
                                  return CircleAvatar(
                                    ...
                                    child: Icon(
                                      BlocProvider.of<FavoritesCubit>(context).isFavorite(products[index].id)
                                          ? Icons.favorite
